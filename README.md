...
Cloudflare Workers + D1 + Cron Trigger + Send Email 的提醒系统。
...
3. 在 GitHub Secrets 配置 `CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID`、`ADMIN_PASSWORD`。
4. 推送到 GitHub `main` 分支。
5. GitHub Actions 自动执行测试、D1 迁移和 `wrangler deploy`。
...
- 邮件发送依赖 Cloudflare Send Email 绑定 `EMAIL`。
